...
class DetectorTF2:
  ...
  def __init__(self, path_to_checkpoint, path_to_labelmap, class_id, threshold):  #class_id=None, threshold=0.5
    # class_id is list of ids for desired classes, or None for all classes in the labelmap
    # 可以指定labelmap內的種類，或是使用預設None代表全部的種類
  ...
  def DisplayDetections(self, image, boxes_list, det_time=None):
    ...
    for idx in range(len(boxes_list)): #!!!
      ...
      x = x_min
      y = int((y_min + y_max)/2)
      cv2.putText(img, text, (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                                                               #^字體大小        ^粗細
    if det_time != None:
